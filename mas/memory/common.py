...
class StateChain:
    ...
    def add_message(self, agent_message: AgentMessage, upstream_agent_ids: list[str]) -> str:
        ...
        current_state: nx.DiGraph = self._get_current_state()  # 这个时候用那个新的

        agent_message_dict: dict = asdict(agent_message) # 转换成一个纯字典形式
        node_id = self._generate_node_id(agent_message.agent_name)  # return f"{agent_name}-{name_counter[agent_name]}"
        current_state.add_node(node_id, **agent_message_dict)  # 添加一个节点(以及这个节点对应的属性数据)
