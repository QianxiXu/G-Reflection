...
class GMemory(MASMemoryBase):
    ...
    def _retrieve_memory_raw(
        ...
        related_point_num: int = max((successful_topk + failed_topk) // 2, 1) # 计算成功和失败项目数量的平均值（向下取整），但无论如何都要保证结果至少为 1。
        ...
        true_tasks_doc_with_score = sort_and_filter_by_similarity(true_tasks_doc, threshold)[:successful_topk] # 拿检索到的文档和原始的 query 进行相似度判断，根据这个相似度进行排序
...
class TaskLayer:
    ...
    def cluster_tasks(self) -> None:
        ...
        try: 
            ...
            c, num_clust, req_c = FINCH(X, distance='cosine')
            labels = c[:, 0]
