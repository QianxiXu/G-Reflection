...
class AlfworldEnv(BaseEnv):
    def __init__(
        ...
        self.max_trials: int = max_trials  # 30 
        self.reset()
        print(f"[AlfworldEnv] Initialized Alfworld environment with max_trials={self.max_trials}")
        # exit(0)
    
    def set_env(self, configs: dict) -> tuple[str, str]:  
        self.gamefile = configs['env_kwargs']['gamefile'] # 一个保存环境数据的路径，包括初始状态和目标任务
        self.env_name: str = configs['env_name']        # look_at_obj
