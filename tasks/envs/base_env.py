...
class BaseRecorder:
    ...
    def __post_init__(self):
        ...
        file_handler = logging.FileHandler(self.file_path)  # 创建一个文件处理器 职责是接收日志 将其写入到指定文件中
        formatter = logging.Formatter('%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S') # 规定日志输出格式， 前面是时间后面是消息
