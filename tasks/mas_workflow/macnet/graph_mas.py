...
#     def schedule(self, task_config: dict) -> tuple[float, bool]:  # 当前节点是0 0 的空间前缀是 1 
#         """
...
#         successful_trajectories, _, insights = self.meta_memory.retrieve_memory(  # 先做一遍检索
...
#         )
#         self.notify_observers('init_user_pxrompt-------->:\n' + user_prompt)
#         self.notify_observers('init_user_pxrompt end --------<\n')
#         # Main loop for task execution
#         for i in range(env.max_trials):
#             self.notify_observers(f'now_the_trial is {i} ----------------------------------------------------------->:\n')
#             upstream_node_ids: dict[str, str] = {}   

#             in_degree = {node.id: len(node.spatial_predecessors) for node in self._agent_nodes.values()}
#             zero_in_degree_queue = [node_id for node_id, deg in in_degree.items() if deg == 0] # 0 入度 的节点，当前是两个 0 入度的点
            
#             # topo sorting
#             while zero_in_degree_queue:  # 执行入度为0的Agent 
#                 current_node_id = zero_in_degree_queue.pop(0) 
#                 self.notify_observers(f'now_the_node agent is {current_node_id} 开始执行 ------->:\n')
#                 curr_node: Node = self._find_agent_node_by_uuid(current_node_id)  # 根据 Id 获取节点
#                 user_prompt: str = format_task_prompt_with_insights(
#                     few_shots=few_shots, 
#                     memory_few_shots=successful_shots,
...
#                 while tries < 3:
#                     try:
#                         action: str = curr_node.execute(user_message, use_critic=self._use_critic, recoder = self.notify_observers)     
#                         if action == '':
#                             continue  
#                         self.notify_observers(f"\nagent {current_node_id} 的执行结果为: {action}\n")
...
#                     message=action
#                 )
#                 self.notify_observers(f"""node agent {current_node_id}'s out is: {action}\n""")
...
#                 self.notify_observers(f'node agent {current_node_id} 执行结束 ------------------<\n\n')
            
#             # Finish one step
#             self._update_memory()  # 更新每个节点的 输入输出
#             self._connect_decision_node()  # 建立不同节点和决策节点的空间连接
#             self.notify_observers(f'---- 开始进行 decision_node 的执行 ------->:\n')
#             action = self._decision_node.execute(user_message, use_critic=False, recoder=self.notify_observers)   
#             self._disconnect_dicision_node() # 删除决策节点的空间连接
#             self.notify_observers("\n决策执行节点的最终结果为-->:\n" + action)  # 记录一下决策 Agent 执行后的结果
...
#             self.notify_observers(f'now_the_trial {i} is end -----------------------------------------------------------<\n\n\n')
#             if done:
#                 break
...
from refine.step1_detact import ParallelGraphBuilder
...
class MacNet(MetaMAS):
    ...
    def schedule(self, task_config: dict, record_json = None, builder: ParallelGraphBuilder = None) -> tuple[float, bool]:  # 当前节点是0 0 的空间前缀是 1 
        """
        Schedules and executes a task based on the given task configuration.
        ...
        record_json 这个东西用来保存每一步的日志 后续用来做分析
        """
        def get_state_graph_upstream_node_ids(node: Node, upstream_node_ids: dict[str, str]) -> list[str]:
            upstream_ids: list[str] = []
            for node in node.spatial_predecessors:
        ...
        # 拿到 advice
        reflection_advice = ""
        if task_config.get('reflection_advice') is not None:
            reflection_advice: str = task_config.get('reflection_advice')
        ...
        if record_json is not None:
            record_json.append(task_description)
        ...
        raw_rules: list[str] = [insight for insight in insights]
        # 是否进行 insight 消融
        if task_config.get('args') is not None and task_config['args'].no_use_insights == True:
            raw_rules = []

        roles_rules: dict[str, list[str]] = self._project_insights(raw_rules)
        
        # 是否进行 successful_shots 消融
        if task_config.get('args') is not None and task_config['args'].no_use_successful_shots == True:
            successful_shots = []
        ...
        user_prompt: str = format_task_prompt_with_insights(
            ...
            insights=raw_rules,
            task_description=self.meta_memory.summarize(upstream_agent_ids=None),
            guidance=reflection_advice if reflection_advice != '' else None,
            reflection=graph_guidance if graph_guidance != '' else None
        )
        self.notify_observers('init_user_pxrompt-------->:\n' + user_prompt)
        self.notify_observers('init_user_pxrompt end --------<\n')
        ...
        for i in range(env.max_trials):
            ...
            step_message: str = f'Act {i + 1}: {action}\nObs {i + 1}: {observation}'
            if record_json is not None:
                record_json.append(step_message)
            self.notify_observers("\n本步骤执行的结果为-->:\n" + step_message)
            ...
            if builder is not None and len(builder.graphs) > 1 and action.strip().lower().startswith("think"):
        ...
        if record_json is not None:
            return final_reward, final_done, record_json    
        return final_reward, final_done    
