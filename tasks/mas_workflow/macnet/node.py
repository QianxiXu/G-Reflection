...
#     def execute(self, user_message: Message, use_critic: bool, recoder = None) -> str:
#         """
...
#         temporal_info: Dict[str, Dict] = self.get_temporal_upstream_info()  # 时间上暂时是 0
#         if recoder is not None:
#             recoder(f'spatial_info for agent {self._id} is :\n {spatial_info}\n')
#             recoder(f'temporal_info for agent {self._id} is :\n {temporal_info}\n')
#             recoder(f"开始进行judge 或者 多答案汇总:\n")
#         user_prompt: str = self._process_inputs(user_message, spatial_info, temporal_info, use_critic, recoder=recoder)  # 进行不同 Agent 的答案汇总，这里没有使用 critic评价，只是汇总/ 
...
#     def _process_inputs(self, user_message: Message, spatial_info: dict, temporal_info: dict, use_critic: bool, recoder = None) -> str:
...
#             recoder(f'## spatial_judge!!!: Agent {uuid}, role is {info['role']}, output is:\n\n {info['output']}\n')
#             # spatial_upstream_message += f"## 被 judge 的Agent为 {uuid}, role is {info['role']}, output is:\n\n '''开始\n {info['output']}'''结束\n"
...
#                 if recoder is not None:
#                     recoder(f'## spatial_critic!!!: Critic\'s Suggestions for Improvement: {critic_message}\n')
           
        
#         for uuid, info in temporal_info.items():  # 
#             recoder(f'## temporal_judge!!!: Agent {uuid}, role is {info['role']}, output is:\n\n {info['output']}\n')
#             # temporal_upstream_message += f"## 被 judge 的Agent为 {uuid}, role is {info['role']}, output is:\n\n '''开始\n {info['output']}'''结束\n"
#             temporal_upstream_message += f"## Agent {uuid}, role is {info['role']}, output is:\n\n{info['output']}\n"
#             if use_critic:
#                 critic_message: str = self._critic_upstream_agent(user_prompt, info['output'])
#                 spatial_upstream_message += f"## Critic's Suggestions for Improvement:{critic_message}\n\n"
#                 if recoder is not None:
#                     recoder(f'## temporal_critic!!!: Critic\'s Suggestions for Improvement: {critic_message}\n')
