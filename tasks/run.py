...
os.environ["OPENAI_API_BASE"] = "https://api.openai-hk.com/v1"
os.environ["OPENAI_API_KEY"] = "hk-10t3l5100005993509f283dc8f4a81a4a55e3d3e191ee8d4"
...
# Prefix = "_test_vision" # 跑了 20 个样例 报错了 TypeError: FINCH() got an unexpected keyword argument 'metric' 
Prefix = "_test_vision_2" # 之前出错了 修改以后重新跑一下
Prefix = "_test_vision_no_GT" # 使用徐乾熙的数据集泡一下，这个数据集没有唯一指定 GT
# Prefix = "_test" # 调试一下为什么出错/ 看看能不能把答案拿出来
...
from mas.llm import LLMCallable, GPTChat, OllamaChat, get_price
...
def build_mas(
    ...
    # llm_model: LLMCallable = OllamaChat(model_name=llm_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, choices=['alfworld', 'fever', 'pddl', 'hotpot'])
    ...
    # dir
    if Prefix!= "":
        WORKING_DIR = os.path.join('./.db', get_model_type(model_type) + f"{Prefix}", task, mas_type, f'{mas_memory_type}')
    else:
